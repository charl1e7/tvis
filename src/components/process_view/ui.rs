    ...
    ui.group(|ui| {
            ...
            MetricType::Cpu => {
                if let Some(cpu_history) = state.history.get_process_cpu_history(state.process_idx) {
                    if !cpu_history.is_empty() {
                        ...
                        plot_metric(ui, format!("cpu_plot_{}", state.process_idx), 100.0, cpu_history, state.history.history_max_points, state.stats.peak_cpu * (1.0 + settings.graph_scale_margin));
                    }
                }
            }
            MetricType::Memory => {
                if let Some(memory_history) = state.history.get_memory_history(state.process_idx) {
                    if !memory_history.is_empty() {
                        ...
                        plot_metric(ui, format!("memory_plot_{}", state.process_idx), 100.0, memory_history, state.history.history_max_points, state.stats.peak_memory_mb * (1.0 + settings.graph_scale_margin));
                    }
                }
            }
        }
        ...
        if !state.stats.processes.is_empty() {
            ui.collapsing("Processes", |ui| {
                ui.horizontal(|ui| {
                    ui.label("Sort by:");
                    if ui.selectable_label(state.sort_type == SortType::AvgCpu, "Average CPU").clicked() {
                        state.sort_type = SortType::AvgCpu;
                    }
                    if ui.selectable_label(state.sort_type == SortType::Memory, "Memory").clicked() {
                            ...
                            match state.current_metric {
                                MetricType::Cpu => {
                                    ...
                                    if let Some(cpu_history) = state.history.get_child_cpu_history(state.process_idx, &process.pid) {
                                        ...
                                        plot_metric(
                                            ui,
                                            format!("child_cpu_plot_{}_{}", state.process_idx, process.pid),
                                            80.0,
                                            cpu_history,
                                ...
                                MetricType::Memory => {
                                    ...
                                    if let Some(memory_history) = state.history.get_child_memory_history(state.process_idx, &process.pid) {
                                        ...
                                        plot_metric(
                                            ui,
                                            format!("child_memory_plot_{}_{}", state.process_idx, process.pid),
                                            80.0,
                                            memory_history,
                                            state.history.history_max_points,
